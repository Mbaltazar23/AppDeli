...
export class MercadoPagoRepositoryImpl implements MercadoPagoRepository {
  ...
  async createdCardToken(
    cardTokenParams: CardTokenParams
  ): Promise<ResponseMercadoPagoCardToken> {
    const response = await ApiMercadoPago.post<ResponseMercadoPagoCardToken>(
      "/card_tokens?public_key==TEST-f1479449-10f9-4b1e-9810-68032589add2",
      cardTokenParams
    );
    return response.data;
  }
  ...
  async getInstallments(
    bin: string,
    amount: number
  ): Promise<ResponseMercadoPagoInstallments> {
    const response = await ApiMercadoPago.get<
      ResponseMercadoPagoInstallments[]
    >(`/payment_methods/installments?bin=${bin}&amount=${amount}`);
